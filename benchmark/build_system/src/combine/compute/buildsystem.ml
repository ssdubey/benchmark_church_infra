...
let find_in_db lib private_branch = 
    ...
    Printf.printf "\nfind_in_db: %f %s" (etime -. stime) lib;
    Lwt.return_true
    with 
    _ -> Lwt.return_false
...
let get_kv x =
    let s = String.split_on_char '/' x in 
    let key = List.hd (List.rev s) in 
    (key,x)

let rec trigger_split lst =
    match lst with 
    |h::t -> get_kv h :: (trigger_split t)
    |[] -> []

let find_file_kv liblistpath lib = 
...
let rec insert_value h ip private_branch_anchor = 
    ...
    let lib, loc = h in 
        ...
        (*print_float (diff);*)
        (* insert_value t ip private_branch_anchor *)
    (* | [] -> () *)
...
let file_to_liblist indexpath = 
    let fileContentBuf = readfile (open_in indexpath) in 
    let liblist = String.split_on_char('\n') fileContentBuf in
    List.iter (fun x -> print_string x) (List.tl (List.rev liblist));
    List.tl (List.rev liblist)
...
let buildLibrary ip client replica liblistpath libindex =
    ...
    let liblist = file_to_liblist (libindex) in (*List.iter (fun x -> print_string x) liblist;*)
