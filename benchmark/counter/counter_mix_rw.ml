...
let mergeOpr_help branchList currentBranch currentBranch_string repo opr_meta =
  ...
  mergeOpr branchList currentBranch currentBranch_string repo opr_meta
...
let getvalue private_branch_anchor lib client get_meta =
    let stime = Unix.gettimeofday() in
        Scylla_kvStore.get private_branch_anchor [lib] >>= fun item -> (*Printf.printf "item = '%d'" (Int64.to_int item);*)
        ignore item;
    let etime = Unix.gettimeofday() in
...
let post_operate_help opr_load private_branch_anchor repo client replica total_opr_load flag set_meta get_meta publish_meta refresh_meta =
  ...
  (* let oc =
    try open_out_bin "/home/shashank/benchmark_church_infra/benchmark/counter/lockfile"
    with Sys_error _ -> Printf.printf "file not found"; raise (File_not_found "file")
  in
  Unix.lockf (Unix.descr_of_out_channel oc) Unix.F_LOCK 0;
  ignore @@ publish_to_public repo client replica publish_meta; 
  Unix.lockf (Unix.descr_of_out_channel oc) Unix.F_ULOCK 0;
  close_out oc; *)
...
let rec operate opr_load private_branch_anchor repo client replica total_opr_load flag done_opr set_meta get_meta publish_meta refresh_meta =
  let rw_load = opr_load/2 in
  
  post_operate_help rw_load private_branch_anchor repo client replica total_opr_load flag set_meta get_meta publish_meta refresh_meta;
  ...
  in *)
  let new_opr_load, flag = 
    if (done_opr + 100) < total_opr_load then
        ((opr_load + 100), true)
    else    
        ((total_opr_load - done_opr), false)
    in
  ...
  (* Printf.printf "\n\nnew operation load: %d  done opr load = %d\n" new_opr_load done_opr; *)
  if flag=true then( (*flag denotes if it is a last round of operation or not. true = more rounds are there, false = no more rounds*)
  (* Printf.printf "\ncalling operate on recursion\n"; *)
      operate new_opr_load private_branch_anchor repo client replica total_opr_load flag done_opr set_meta get_meta publish_meta refresh_meta)
  else  if new_opr_load != 0 then( 
    (* Printf.printf "\nrecursion on operate over\n"; *)
      let rw_load = new_opr_load/2 in
      
      post_operate_help rw_load private_branch_anchor repo client replica total_opr_load flag set_meta get_meta publish_meta refresh_meta
  
  )else ()
  (* (Printf.printf "\nrecursion on operate over and load is 0\n";) *)
    
  
let buildLibrary ip client replica total_opr_load set_meta get_meta publish_meta refresh_meta =
  ...
  let opr_load = 100 in 
  let done_opr = 100 in
  operate opr_load private_branch_anchor repo client replica total_opr_load true done_opr set_meta get_meta publish_meta refresh_meta;
...
let singleread ip client set_meta get_meta = 
  let conf = Irmin_scylla.config ip in
  Scylla_kvStore.Repo.v conf >>= fun repo ->
  
  create_or_get_private_branch repo client >>= fun private_branch_anchor ->
  (* ignore @@ build ["ee"] private_branch_anchor repo client set_meta get_meta "post_write"; *)
  let stime = Unix.gettimeofday() in
  ignore @@ build ["ee"] private_branch_anchor repo client set_meta get_meta "read";
  let etime = Unix.gettimeofday() in
    Printf.printf "read time: %f" (etime -. stime);
    Lwt.return_unit
...
let _ =
  ...
  ignore @@ buildLibrary hostip client replica (int_of_string total_opr_load) set_meta get_meta publish_meta refresh_meta;
  (* ignore @@ singleread hostip client set_meta get_meta; *)
